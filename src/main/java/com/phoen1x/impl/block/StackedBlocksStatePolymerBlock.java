...
public record StackedBlocksStatePolymerBlock(Map<BlockState, BlockState> map, FactoryBlock fallback) implements FactoryBlock, PolymerTexturedBlock, BSMMParticleBlock {

    public static StackedBlocksStatePolymerBlock of(Block block, BlockModelType type) {
        return of(block, type, StackedBlocksBaseFactoryBlock.BARRIER, x -> true);
    }
    public static StackedBlocksStatePolymerBlock of(Block block, BlockModelType type, FactoryBlock fallback, Predicate<BlockState> canUseBlock) {
        var id = Registries.BLOCK.getId(block);

        var path = FabricLoader.getInstance().getModContainer("stackedblocks").get()
        ...
        try {
            ...
            return new StackedBlocksStatePolymerBlock(map, fallback);
        } catch (Throwable e) {
            return null;
        }
    }
